...
class Phase(object):
    ...
    def __init__(self, trend_df, share_df):
    ...
    def merge_macd(self):
        one = dict()
        trend_len = len(self.trend_df)
        for index, row in self.trend_df.iterrows():
            ...
            if index == trend_len - 1:
                break
            next_row = self.trend_df.iloc[index + 1]

            if len(one) == 0:
                one[MACD_STATUS] = row[action.INDEX_PHASE_STATUS]
                one[MACD_START] = row[action.INDEX_VALUE]
                one[MACD_AREA] = abs(row[action.INDEX_VALUE])
            ...
            if next_row[action.INDEX_PHASE_STATUS] != one[MACD_STATUS] or (self._check_position(one[MACD_START], next_row[action.INDEX_VALUE])):
                one[MACD_END] = row[action.INDEX_VALUE]
