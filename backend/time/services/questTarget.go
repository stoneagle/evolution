...
func (s *QuestTarget) BatchAdd(targets []models.QuestTarget) (err error) {
	...
	for _, one := range targets {
		has, err := s.Engine.Where("quest_id = ?", one.QuestId).And("entity_id = ?", one.EntityId).Get(new(models.QuestTarget))
		if err != nil {
			return err
		}
		if !has {
...
func (s *QuestTarget) ListWithCondition(questTarget *models.QuestTarget) (questTargets []models.QuestTarget, err error) {
	...
	sql := s.Engine.Unscoped().Table("quest_target").Join("INNER", "entity", "entity.id = quest_target.entity_id").Join("INNER", "area", "area.id = entity.area_id")
	...
	for _, one := range questTargetsJoin {
		one.QuestTarget.Entity = one.Entity
		one.QuestTarget.Entity.Area = one.Area
